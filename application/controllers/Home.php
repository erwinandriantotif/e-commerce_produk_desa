...
class Home extends CI_Controller {
	...
	function midtranspay(){
		...
		if(isset($_GET["status"]) AND $_GET["status"] == "success"){
			...
			";
			$this->func->sendEmailOK($usr->username,$toko->nama." - Pesanan",$pesan,"Pesanan");
			$pesan = "
				Halo *".$usr->nama."* \n".
		...
		if(isset($_GET["params"]) AND $_GET["params"] == "notify"){
			if(isset($_POST["status"]) AND $_POST["status"] == "berhasil"){
				...
				$pesan = "
					...
					"<a href='".site_url("manage/pesanan")."'>PESANANKU &raquo;</a>
				";
				$this->func->sendEmailOK($usrid->username,$toko->nama." - Pesanan",$pesan,"Pesanan");
				$pesan = "
					Halo *".$usr->nama."* \n".
					"Terimakasih, pembayaran untuk pesananmu sudah kami terima. \n".
					"Mohon ditunggu, admin kami akan segera memproses pesananmu \n".
		...
		if($type == "challenge"){
			...
			if($db->num_rows() == 0){
				...
				foreach($db->result() as $res){
					...
					";
					$this->func->sendEmailOK($res->username,$set->nama." - OTP Login",$pesan,"OTP Login");
					$pesan = "
						*PERHATIAN!* \n".
			...
			if($db->num_rows() == 0){
				...
				foreach($db->result() as $res){
					...
					$pesan = "
						...
						"Untuk MASUK KE AKUN ".strtoupper(strtolower($set->nama)).", masukkan kode RAHASIA: <b>".$otp."</b>
					";
					$this->func->sendEmailOK($this->func->getUser($res->usrid,"username"),$set->nama." - OTP Login",$pesan,"OTP Login");
					$pesan = "
						*PERHATIAN!* \n".
						"JANGAN BERIKAN kode ini kepada siapa pun, TERMASUK TIM ".strtoupper(strtolower($set->nama))."\n".
						"WASPADA PENIPUAN! \n".
		...
		if($type == "challenge"){
			...
			if(strpos($_POST["email"],"@") !== false){
				$pesan = "
					...
					"Untuk MASUK KE AKUN ".strtoupper(strtolower($set->nama)).", masukkan kode RAHASIA: <b>".$otp."</b>
				";
				$this->func->sendEmailOK($_POST["email"],$set->nama." - OTP Login",$pesan,"OTP Login");
			}else{
				$pesan = "
					*PERHATIAN!* \n".
					"JANGAN BERIKAN kode ini kepada siapa pun, TERMASUK TIM ".strtoupper(strtolower($set->nama))."\n".
				...
				foreach($db->result() as $res){
					...
					if(strpos($res->emailhp,"@") !== false){
						$pesan = "
							...
							"Untuk MASUK KE AKUN ".strtoupper(strtolower($set->nama)).", masukkan kode RAHASIA: <b>".$otp."</b>
						";
						$this->func->sendEmailOK($res->emailhp,$set->nama." - OTP Login",$pesan,"OTP Login");
					}else{
						$pesan = "
							*PERHATIAN!* \n".
							"JANGAN BERIKAN kode ini kepada siapa pun, TERMASUK TIM ".strtoupper(strtolower($set->nama))."\n".
	...
	function signup($pwreset="none"){
		if(isset($_GET["verify"])){
			...
			if($db->num_rows() > 0){
				echo json_encode(array("success"=>false,"message"=>"alamat email/no handphone sudah terdaftar"));
			}else{
				echo json_encode(array("success"=>true,"message"=>""));
			}
