...
class Global_data extends CI_Model{
	...
	function resetPass($email){
		...
		$pesan = '
			...
			<head>
				...
				.alink{text-decoration:none;background:#007BFF;padding:10px 24px;border-radius:3px;margin-bottom:20px;}
				</style>
			</head>
			<body>
				<div class="border">
		...
		$pesanWA = '
			...
			';
		if($this->sendEmailOK($user->username,$this->getSetting("nama"),$pesan,"Reset password")){
			$this->sendWAOK($profil->nohp,$pesanWA);
			return true;
		}else{
			return false;
		}
	...
	function verifEmail($id=0){
		if($id > 0){
			...
			if($this->sendEmailOK($user->username,$this->getSetting("nama"),$pesan,$subyek)) {
				return true;
			} else {
				return false;
			}
	...
	function verifWA($id=0){
		if($id > 0){
			...
			if($this->sendWAOK($profil->nohp,$pesan)) {
				return true;
			} else {
				return false;
			}
