			...
			<div class="p-b-50 t-center">
				<div class="bread-crumb">
					<div class="cat-button">
						<a href="<?=site_url("shop/")?>" class="btn btn-primary bo-rad-23 p-l-16 p-r-16 m-r-4 m-b-12">Semua kategori
						...
						<?php 
							...
							foreach($db->result() as $r){
								$select = ($r->id == $idcat) ? "btn btn-success bo-rad-23 p-l-16 p-r-16 m-r-4 m-b-12" : "btn btn-primary bo-rad-23 p-l-16 p-r-16 m-r-4 m-b-12";
						?>
							<a href="<?=site_url("kategori/".$r->url)?>" class="<?=$select?>">
			...
			<div class="p-b-50">
					...
					<div class="row produk-wrap">
						<?php
							...
							$where = "(nama LIKE '%$cari%' OR harga LIKE '%$cari%' OR deskripsi LIKE '%$cari%') AND status = 1 AND stok > 0 AND idcat = ".$idcat;
							$this->db->where($where);
							if(count($notin) > 0){
							...
							foreach($db->result() as $r){
								...
								if($totalstok > 0){
									$totalproduk += 1;
						?>
							<div class="col-6 col-md-3 m-b-30 cursor-pointer produk-item" onclick="window.location.href='<?php echo site_url('produk/'.$r->url); ?>'">
								<!-- Block2 -->
                      ...
                      <div class="bottom-area d-flex px-3">
						...
						<?php
							...
							if($db->num_rows() == 0 OR $totalproduk == 0){
								echo "<div class='col-12 text-center m-tb-40 bg-danger text-primary'><h2>Stok Kosong / Belum ada Produk</h2></div>";
							}
						?>
					</div>
