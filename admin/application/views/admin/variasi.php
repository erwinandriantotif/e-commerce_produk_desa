...
<script type="text/javascript">
	$(function(){
		...
		$("#simpan").on("submit",function(e){
			...
			$.post("<?=site_url("Admin/tambahvariasi")?>",$(this).serialize(),function(e){
				var data = eval("("+e+")");
				$("#submit").html(btn);
	...
	function hapusSize(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("Admin/hapussize")?>",{"theid":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
	...
	function hapusWarna(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("Admin/hapuswarna")?>",{"theid":id},function(msg){
					var data = eval("("+msg+")");
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("Admin/variasi")?>";
	...
	<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="modalLagu" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<form id="simpan" action="<?=site_url("Admin/tambahvariasi")?>" method="POST">
